...
import { AppMember, MemberAdminService, WorkspaceAdminDetails } from '../../../../../services/member-admin.service';
...
export class MemberAdminComponent implements OnInit {
    ...
    selectedWorkspaceId = ''; workspaceDetails: WorkspaceAdminDetails | null = null; workspaceLoading = false; workspaceError = '';
    ...
    formatDateTime(value: string): string { const date = new Date(value); return Number.isNaN(date.getTime()) ? '-' : date.toLocaleString('ko-KR'); }
    async toggleWorkspaceDetails(workspaceId: string): Promise<void> {
        if (this.selectedWorkspaceId === workspaceId) { this.selectedWorkspaceId = ''; this.workspaceDetails = null; this.workspaceError = ''; return; }
        this.selectedWorkspaceId = workspaceId; this.workspaceDetails = null; this.workspaceError = ''; this.workspaceLoading = true;
        try { this.workspaceDetails = await this.memberAdmin.workspaceDetails(workspaceId); }
        catch (error) { this.workspaceError = error instanceof Error ? error.message : '워크스페이스 상세 정보를 불러오지 못했습니다.'; }
