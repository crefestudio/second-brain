...
export class PurchaserAdminComponent implements OnInit {
    ...
    sortKey: SortKey = 'purchasedAt'; memberFilter: MemberFilter = 'all'; productOptionFilter: ProductOptionFilter = 'all'; registrationFilter: RegistrationFilter = 'all'; notifyFilter: NotifyFilter = 'all';
    ...
    get filteredPurchasers(): LifeupPurchaser[] { const source = [...this.purchasers, ...this.csvOnly]; const memberFiltered = this.memberFilter === 'all' ? source : source.filter(p => p.memberType === this.memberFilter); const productOptionFiltered = this.productOptionFilter === 'all' ? memberFiltered : memberFiltered.filter(p => this.productOption(p) === this.productOptionFilter); const registrationFiltered = this.registrationFilter === 'all' ? productOptionFiltered : productOptionFiltered.filter(p => this.registration(p) === this.registrationFilter); const filtered = this.notifyFilter === 'all' ? registrationFiltered : registrationFiltered.filter(p => this.notifyFilter === 'yes' ? p['notify'] === '예' : p['notify'] === '아니오'); return [...filtered].sort((a, b) => this.sortKey === 'purchasedAt' ? this.timestamp(b) - this.timestamp(a) : String(a[this.sortKey] || '').localeCompare(String(b[this.sortKey] || ''), 'ko')); }
